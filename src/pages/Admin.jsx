...
export default function Admin() {
    ...
    const handleSave = async (e) => {
        e.preventDefault();
        setLoading(true);
        const toastId = toast.loading("Uploading images and saving data...");
        ...
        try {
            ...
            if (galleryFiles.length > 0) {
                const newGalleryUrls = await Promise.all(
                    Array.from(galleryFiles).map(file => uploadToCloudinary(file))
                );
    ...
    return (
        <div className="max-w-2xl mx-auto p-8 bg-white rounded-[3rem] shadow-2xl mt-10 mb-20">
            ...
            <form onSubmit={handleSave} className="space-y-6">
                ...
                <div className="p-6 bg-blue-50 border-2 border-dashed border-blue-200 rounded-[2rem]">
                    <label className="block text-xs font-black uppercase text-blue-600 mb-2 tracking-tighter">Main Profile Photo</label>
                    <input type="file" onChange={(e) => setMainImageFile(e.target.files[0])} className="text-sm font-bold w-full" />
                    {existingMain && <img src={existingMain} className="w-20 h-20 mt-2 rounded-xl object-cover border-2 border-white shadow-sm" alt="Current" />}
                </div>

                <div className="p-6 bg-orange-50 border-2 border-dashed border-orange-200 rounded-[2rem]">
                    <label className="block text-xs font-black uppercase text-orange-600 mb-2 tracking-tighter">Gallery Images (Multiple)</label>
                    <input type="file" multiple onChange={(e) => setGalleryFiles(e.target.files)} className="text-sm font-bold w-full" />
                    <div className="flex gap-2 mt-2 overflow-x-auto">
                        {existingGallery.map((img, i) => (
                            <img key={i} src={img} className="w-12 h-12 rounded-lg object-cover border border-white shadow-sm flex-shrink-0" alt="gallery" />
                        ))}
                    </div>
                </div>
                ...
                <div className="space-y-1">
                    ...
                    <textarea value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="Tell us about the product..." className="w-full p-4 bg-slate-50 rounded-2xl outline-none h-32 font-bold resize-none focus:ring-2 ring-orange-100" required />
                </div>
